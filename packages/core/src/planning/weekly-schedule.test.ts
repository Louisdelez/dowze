...
describe('weeklySchedule', () => {
  const base = { age: 14, activeDays: [1, 2, 3, 4, 5], dayStartMin: 8 * 60, dayEndMin: 17 * 60, intensity: 'moyen' as const, hasSecondary: true };
  ...
  it('protège langue + révisions même dans un créneau du soir très court (compression)', () => {
    const soir = weeklySchedule({ ...base, dayStartMin: 19 * 60, dayEndMin: 20 * 60, intensity: 'leger' });
  ...
  it('la passion n’est jamais le premier bloc de la journée', () => {
    ...
    for (const dow of base.activeDays) {
      const first = blocks.filter((b) => b.dayOfWeek === dow).sort((a, b) => a.startMin - b.startMin)[0];
...
describe('weeklySchedule — orchestration multi-source (activités récurrentes de plugins)', () => {
  const base = { age: 20, activeDays: [1, 2, 3, 4, 5], dayStartMin: 8 * 60, dayEndMin: 18 * 60, intensity: 'moyen' as const, hasSecondary: false };
  const workout = {
    key: 'fitness:w1',
  ...
  it('inscrit « sport 3×/sem » sur exactement 3 jours ESPACÉS', () => {
    const blocks = weeklySchedule({ ...base, recurring: [{ ...workout, cognitiveBoostBeforeStudy: true }] });
    const days = [...new Set(blocks.filter((b) => b.type === 'plugin').map((b) => b.dayOfWeek))].sort();
  ...
  it('respecte le cap OMS (weeklyCapMin) en réduisant la fréquence', () => {
    ...
    const blocks = weeklySchedule({ ...base, recurring: [{ ...workout, frequencyPerWeek: 5, weeklyCapMin: 100 }] });
  ...
  it('ne place jamais d’activité un jour de repos (non actif)', () => {
    ...
    expect(blocks.some((b) => b.type === 'plugin' && (b.dayOfWeek === 0 || b.dayOfWeek === 6))).toBe(false);
  });

  it('n’évince jamais langue + révisions (l’étude reste le socle)', () => {
    const blocks = weeklySchedule({ ...base, recurring: [{ ...workout, frequencyPerWeek: 5, durationMin: 90 }] });
    for (const dow of base.activeDays) {
