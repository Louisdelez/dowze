...
import { CompanionApi, type CompanionAgent, type DowzeClient } from '@dowze/api-client';
...
export function CompanionDock({
    ...
    (async () => {
      try {
        ...
        const lists = await Promise.all([
          api.current!.listAgents('home'),
          ...spaces.map((s) => api.current!.listAgents(s.id)),
        ]);
                  ...
                  [primary.id]: [
                    {
                      ...
                      from: 'them',
                      text:
                        primary.personality?.greeting ||
                        'Coucou ! Je suis là, pour tout ce que tu veux.',
                    },
                  ],
                },
          );
  ...
  useEffect(() => {
    ...
    if (!a || !(a.mode === 'agent' || a.mode === 'relay' || (a.space === 'home' && !a.isPrimary)))
      return;
    api
      .current!.messages(sel)
      .then((msgs) => {
  ...
  const push = useCallback((id: string, text: string, from: 'me' | 'them', meta?: boolean) => {
    setConvs((prev) => ({
      ...prev,
      [id]: [...(prev[id] ?? []), { id: mid.current++, from, text, meta }],
    }));
  }, []);
  ...
  const send = useCallback(() => {
    ...
    const err = () => {
      push(sel, '(Je ne peux pas répondre là — vérifie la clé IA du Copilote.)', 'them');
      done();
    };
    if (a.mode === 'relay') {
      api
        .current!.relaySay(txt)
        .then(done)
        .catch(() => {
          push(sel, '(Instruction non transmise, réessaie.)', 'them');
          done();
        });
    } else if (a.space === 'home') {
      // Compagnon de la MAISON = LEADER : il répond lui-même OU mobilise/crée des abeilles des open-spaces.
      api
        .current!.orchestrate(txt, a.isPrimary ? undefined : a.id, {
          service,
        ...
        .then((r) => {
          ...
          if (r.delegates?.length)
            push(sel, `a mobilisé : ${r.delegates.map((d) => d.name).join(', ')}`, 'them', true);
          done();
        })
        .catch(err);
      ...
      window.setTimeout(() => {
        push(sel, 'Bien reçu, je m’en occupe !', 'them');
        done();
      }, 500);
    }
  }, [draft, sel, agents, push, service]);
  ...
  return (
    <div className={`fixed bottom-4 z-[900] ${pos}`}>
      {open && (
        <div className="mb-3 flex h-[520px] max-h-[80vh] w-[min(92vw,380px)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-100">
              {selAgent?.skinUrl && (
                <span className="scale-[0.42]">
                  <CodexPet url={selAgent.skinUrl} size={96} />
                </span>
              )}
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold text-slate-800">
                {selAgent?.name ?? 'Compagnon'}
              </div>
              <div className="truncate text-[11px] text-slate-400">
                {selAgent?.isPrimary
                  ? 'Ta ruche — je mobilise l’équipe'
                  : selAgent?.role ||
                    (selAgent?.mode === 'relay' ? 'Relais Claude Code / Codex' : 'Compagnon')}
              </div>
            </div>
            <button
            ...
            <div className="flex shrink-0 gap-1.5 overflow-x-auto border-b border-slate-100 px-3 py-2">
              {agents.map((a) => (
                <button
                  ...
                  onClick={() => setSel(a.id)}
                  title={a.name}
                  className={`flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border-2 transition ${sel === a.id ? 'border-slate-800' : 'border-transparent bg-slate-100 hover:border-slate-300'}`}
                >
                  {a.skinUrl ? (
                    <span className="scale-[0.4]">
                      <CodexPet url={a.skinUrl} size={96} />
                    </span>
                  ) : (
          ...
          <div className="min-h-0 flex-1 space-y-2 overflow-y-auto bg-slate-50 px-3 py-3">
            {(sel ? (convs[sel] ?? []) : []).map((m) =>
              m.meta ? (
                <div key={m.id} className="text-center text-[10px] italic text-slate-400">
                  · {m.text} ·
                </div>
              ) : (
                <div
                ...
                >
                  <div
                    className={`max-w-[80%] rounded-2xl px-3 py-1.5 text-sm ${m.from === 'me' ? 'bg-slate-800 text-white' : 'bg-white text-slate-700 shadow-sm'}`}
                  >
                    {m.text}
                  </div>
                </div>
            ...
            {busy && (
              <div className="flex justify-start">
                <div className="rounded-2xl bg-white px-3 py-1.5 text-sm text-slate-400 shadow-sm">
                  …
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-2 border-t border-slate-100 px-3 py-2">
            ...
            <button
              ...
              disabled={!draft.trim()}
              aria-label="Envoyer"
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-800 text-white transition hover:bg-slate-700 disabled:opacity-40"
            >
              <svg
                width="16"
                height="16"
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Mon compagnon"
        className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-white shadow-xl ring-1 ring-slate-200 transition hover:scale-105"
      >
        <span className="scale-[0.6]">
          <CodexPet url={primaryUrl} size={96} />
        </span>
      </button>
    </div>
  );
