...
export function TopBar() {
  ...
  return (
    <header className="relative z-30 flex items-center justify-between px-5 py-4 sm:px-8">
      ...
      <a href="/" className="box-logo text-xl sm:text-2xl">
        Dowze
      </a>
      ...
      <nav className="flex items-center gap-1 sm:gap-2">
              ...
              {SERVICES.map((s) => {
                const inner = (
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      {s.soon && (
                        <span className="bg-dowze-red px-1.5 py-0.5 text-[10px] font-extrabold uppercase italic tracking-wide text-white">
                          Bientôt
                        </span>
                      )}
                    </div>
                ...
                return s.href ? (
                  <a
                    key={s.name}
                    href={s.href}
                    className="block px-3 py-2 transition hover:bg-black hover:text-white"
                  >
                    {inner}
                  </a>
                ) : (
                  <div key={s.name} className="block cursor-default px-3 py-2 opacity-55">
                    {inner}
                  </div>
                );
              })}
        ...
        <a
          href="/a-propos"
          className="px-3 py-2 text-sm font-bold uppercase italic tracking-tight text-black transition hover:text-dowze-red"
        >
          À propos
        </a>
        ...
        {!ready ? (
          ...
          <div className="relative" onMouseLeave={() => setProfileOpen(false)}>
            <button
              ...
              className="flex items-center gap-2 border-2 border-black bg-white py-1 pl-1 pr-3 transition hover:border-dowze-red"
            >
              <span className="box-logo px-2 py-1 text-sm">{initial}</span>
              <span className="max-w-[8rem] truncate text-sm font-extrabold uppercase italic tracking-tight text-black">
                {label}
              </span>
              <svg
                width="13"
            ...
            {profileOpen && (
              <div className="absolute right-0 top-full z-40 mt-1 w-52 border-2 border-black bg-white p-1.5">
                <div className="px-3 py-2 text-xs font-bold uppercase italic tracking-tight text-black/50">
                  Mon compte
                </div>
                <a
                  href={APP}
                  className="block px-3 py-2 text-sm font-extrabold uppercase italic transition hover:bg-black hover:text-white"
                >
                  Ouvrir Dowze
                </a>
                <button
                  onClick={logout}
                  className="block w-full px-3 py-2 text-left text-sm font-extrabold uppercase italic text-dowze-red transition hover:bg-dowze-red hover:text-white"
                >
                  Se déconnecter
                </button>
              </div>
          ...
          <>
            ...
            <a
              href="/inscription"
              className="bg-dowze-red px-4 py-2 text-sm font-extrabold uppercase italic tracking-tight text-white transition hover:bg-black"
            >
              Créer un compte
            </a>
          </>
