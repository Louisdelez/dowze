...
export function buildPrompt(a: Asset): string {
  ...
  let view = 'true isometric 3/4 top-down game angle (~30° tilt), orthographic — no perspective distortion';
  let compo = 'one isolated object only, centered, floating, generous padding, sharp clean silhouette';
  if (a.kind === 'wallprop') {
...
/* ---------- Le catalogue ---------- */
const A = (id: string, label: string, category: string, desc: string, kind: AssetKind = 'prop'): Asset => ({ id, label, category, kind, desc });

export const ASSETS: Asset[] = [
  // ===== PORTES & FENÊTRES (objets muraux) =====
  A('fenetre', 'Fenêtre', 'ouvertures', 'a cozy square window with a wooden frame and light curtains', 'wallprop'),
  A('fenetre-ronde', 'Fenêtre ronde', 'ouvertures', 'a cute round porthole-style window with a wooden frame', 'wallprop'),
  A('baie-vitree', 'Baie vitrée', 'ouvertures', 'a large glass sliding bay window with a slim wooden frame', 'wallprop'),
  A('porte-bois', 'Porte en bois', 'ouvertures', 'a cute wooden door with a round knob', 'wallprop'),
  A('porte-vitree', 'Porte vitrée', 'ouvertures', 'a wooden door with a glass upper panel', 'wallprop'),

  // ===== CHAMBRE =====
  A('lit-simple', 'Lit simple', 'chambre', 'small cozy single bed with a cream duvet and one pillow'),
  A('lit-double', 'Lit double', 'chambre', 'plush double bed with a sage quilt and two soft pillows'),
  A('lit-baldaquin', 'Lit à baldaquin', 'chambre', 'cute four-poster canopy bed with light gauzy dusty-rose drapes'),
  ...
  A('coiffeuse', 'Coiffeuse', 'chambre', 'vanity dressing table with an oval mirror and a small stool'),
  A('lampe-chevet', 'Lampe de chevet', 'chambre', 'small bedside table lamp with a warm cream shade'),
  A('etagere-livres', 'Étagère à livres', 'chambre', 'a wall-mounted shelf with a few colorful books and a plant', 'wallprop'),
  A('coffre-jouets', 'Coffre à jouets', 'chambre', 'wooden toy chest with a rounded lid'),
  A('tapis-rond', 'Tapis rond', 'chambre', 'round woven area rug with concentric rings in cream, honey and sage', 'rug'),
  ...
  A('television', 'Télévision', 'salon', 'chunky retro television set with rounded corners and antennae'),
  ...
  A('plante-salon', 'Plante d’intérieur', 'salon', 'leafy potted monstera houseplant in a terracotta pot'),
  A('horloge-murale', 'Horloge murale', 'salon', 'a round vintage wall clock with a wooden rim', 'wallprop'),
  A('tapis-salon', 'Tapis rectangulaire', 'salon', 'rectangular woven rug with a soft geometric pattern in warm pastels', 'rug'),

  // ===== CUISINE =====
  A('plan-travail', 'Plan de travail', 'cuisine', 'wooden kitchen counter with a small cream countertop'),
  A('evier', 'Évier', 'cuisine', 'kitchen sink cabinet with a metal faucet'),
  A('cuisiniere', 'Cuisinière', 'cuisine', 'retro cream kitchen stove with four burners and an oven'),
  ...
  A('placard-haut', 'Placard mural', 'cuisine', 'a wall-mounted wooden kitchen cupboard', 'wallprop'),
  A('micro-ondes', 'Micro-ondes', 'cuisine', 'small cream microwave oven'),
  A('ilot-central', 'Îlot central', 'cuisine', 'kitchen island with a butcher-block top and two stools'),
  A('etagere-epices', 'Étagère à épices', 'cuisine', 'a small wall-mounted spice shelf with tiny colorful jars', 'wallprop'),
  ...
  A('ordinateur', 'Ordinateur', 'bureau', 'chunky retro desktop computer with a rounded monitor and keyboard'),
  ...
  A('bibliotheque-bureau', 'Étagère bureau', 'bureau', 'compact wooden shelf unit with books and boxes'),

  // ===== DÉCO & ACCESSOIRES =====
  A('coussin', 'Coussin', 'deco', 'soft square throw cushion in dusty rose'),
  A('guirlande', 'Guirlande lumineuse', 'deco', 'a string of warm fairy lights (small bulbs) to hang on a wall', 'wallprop'),
  ...
  A('cadre-photo', 'Cadre photo', 'deco', 'a small framed picture with a cute landscape, to hang on a wall', 'wallprop'),
  A('tableau-art', 'Tableau', 'deco', 'a framed wall painting of soft rolling hills, to hang on a wall', 'wallprop'),
  A('plante-suspendue', 'Plante suspendue', 'deco', 'a hanging planter with trailing green vines, to hang on a wall', 'wallprop'),
  ...
  A('horloge-coucou', 'Horloge coucou', 'deco', 'a cute cuckoo wall clock in carved honey wood, to hang on a wall', 'wallprop'),

  // ===== JARDIN (extérieur) =====
  A('banc-jardin', 'Banc de jardin', 'jardin', 'wooden garden bench'),
  A('table-pique-nique', 'Table de pique-nique', 'jardin', 'wooden picnic table with attached benches'),
  ...
  A('pot-fleurs-ext', 'Jardinière', 'jardin', 'long terracotta planter box full of colorful flowers'),
  A('lampadaire-jardin', 'Lanterne de jardin', 'jardin', 'cute vintage garden lamp post with a warm glow'),
  ...
  A('planche-surf', 'Planche de surf', 'plage', 'cute surfboard standing upright with a pastel wave pattern'),
  ...
  A('bosquet-fleurs', 'Massif de fleurs', 'nature', 'small flower bed patch with mixed pastel blooms'),
  ...
  A('bol-nouilles', 'Bol de nouilles', 'nourriture', 'steaming bowl of ramen noodles with chopsticks'),
  A('sushi', 'Sushi', 'nourriture', 'cute pair of nigiri sushi pieces'),
  A('croissant', 'Croissant', 'nourriture', 'golden flaky croissant'),
  A('chocolat-chaud', 'Chocolat chaud', 'nourriture', 'mug of hot chocolate with whipped cream and marshmallows'),
  ...
  A('bureau-travail', 'Bureau', 'travail', 'modern office desk with a light wood top and slim metal legs'),
  A('chaise-bureau-pro', 'Chaise de bureau (pro)', 'travail', 'ergonomic office swivel chair on castors, sage mesh back'),
  A('ordinateur-pro', 'Ordinateur (pro)', 'travail', 'desktop computer with a flat monitor, keyboard and mouse'),
  A('double-ecran', 'Double écran', 'travail', 'dual-monitor computer setup on a desk stand'),
  A('ordinateur-portable', 'Ordinateur portable', 'travail', 'open laptop computer with a glowing screen'),
  ...
  A('fontaine-eau', 'Fontaine à eau', 'travail', 'office water cooler with an upside-down blue bottle'),
  A('corbeille-papier', 'Corbeille à papier', 'travail', 'small metal mesh waste-paper bin'),
  A('pile-dossiers', 'Pile de dossiers', 'travail', 'neat stack of colorful paper folders and documents'),
  A('telephone-bureau', 'Téléphone de bureau', 'travail', 'retro office desk phone with a coiled cord'),
  A('lampe-bureau-pro', 'Lampe de bureau (pro)', 'travail', 'adjustable desk task lamp in soft cream'),
  A('serveur-informatique', 'Serveur informatique', 'travail', 'small server rack tower with blinking status lights'),
  A('projecteur', 'Vidéoprojecteur', 'travail', 'compact ceiling-style video projector'),
  A('plante-bureau-pro', 'Plante de bureau (haute)', 'travail', 'tall potted office plant in a minimalist white pot'),
  A('tableau-blanc-mural', 'Tableau blanc (mural)', 'travail', 'a wall-mounted whiteboard with a marker tray and faint sketches', 'wallprop'),
  A('tableau-liege', 'Tableau en liège', 'travail', 'a cork pin-board with sticky notes and pinned papers', 'wallprop'),
  A('horloge-bureau', 'Horloge de bureau', 'travail', 'a plain round office wall clock', 'wallprop'),
  ...
  A('distributeur-boissons', 'Distributeur de boissons', 'cantine', 'tall vending machine full of colorful drink cans'),
  A('distributeur-snacks', 'Distributeur de snacks', 'cantine', 'snack vending machine with spiral shelves'),
  A('machine-cafe', 'Machine à café', 'cantine', 'office coffee machine dispensing into a small cup'),
  ...
  A('comptoir-self', 'Comptoir self-service', 'cantine', 'stainless self-service canteen counter with a food tray rail'),
  ...
  A('menu-mural', 'Menu mural', 'cantine', 'a wall menu board with a simple daily menu', 'wallprop'),

  // ===== TOILETTES (entreprise / sanitaires) =====
  A('wc-cuvette', 'Cuvette WC', 'sanitaires', 'clean white toilet bowl with a cistern'),
  A('cabine-wc', 'Cabine WC', 'sanitaires', 'toilet stall cubicle with a light partition and a door'),
  ...
  A('miroir-toilettes', 'Miroir', 'sanitaires', 'a rectangular wall mirror with a slim frame', 'wallprop'),
  A('seche-mains', 'Sèche-mains', 'sanitaires', 'a wall-mounted electric hand dryer', 'wallprop'),
  A('distributeur-savon', 'Distributeur de savon', 'sanitaires', 'a small wall soap dispenser', 'wallprop'),
  A('distributeur-papier', 'Distributeur de papier', 'sanitaires', 'a wall paper-towel dispenser', 'wallprop'),
  ...
  A('borne-arcade', 'Borne d’arcade', 'repos', 'retro upright arcade cabinet with a glowing screen'),
  A('table-ping-pong', 'Table de ping-pong', 'repos', 'blue ping-pong table with a net and two paddles'),
  ...
  // ===== GARAGE & PARKING =====
  A('voiture', 'Voiture', 'garage', 'cute rounded compact car, pastel teal, seen at the isometric angle'),
  ...
  A('borne-recharge', 'Borne de recharge', 'garage', 'electric-car charging station pillar with a cable'),
  ...
  A('plot-signalisation', 'Cône de signalisation', 'garage', 'orange and white traffic safety cone'),
  A('place-parking', 'Place de parking', 'garage', 'painted parking space floor markings, white lines on grey asphalt', 'rug'),
  A('panneau-parking', 'Panneau parking', 'garage', 'a blue square P parking sign on a post', 'wallprop'),
];
