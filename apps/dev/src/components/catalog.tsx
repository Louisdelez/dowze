...
import { ASSETS, CATEGORIES, KIND_LABEL, METHOD_NOTE, assetsByCategory, buildPrompt, buildRefPrompt, fileName, type Asset, type Category } from '@/lib/assets';
...
function CopyBtn({ text, label, primary, big }: { text: string; label: string; primary?: boolean; big?: boolean }) {
...
function AssetCard({ asset, done, onToggle, onOpen }: { asset: Asset; done: boolean; onToggle: (v: boolean) => void; onOpen: () => void }) {
  const name = fileName(asset);
  const prompt = useMemo(() => buildPrompt(asset), [asset]);
  return (
    <div className={`flex flex-col rounded-xl border border-border bg-surface p-3 transition ${done ? 'opacity-55' : 'hover:border-foreground/30'}`}>
      <div className="flex items-center gap-2">
        ...
        />
        <span className={`flex-1 truncate text-sm font-semibold ${done ? 'line-through' : ''}`} title={asset.label}>
          {asset.label}
        </span>
        <button
          type="button"
      ...
      <span className="mt-1.5 inline-flex w-fit items-center rounded-full bg-surface-soft px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{KIND_LABEL[asset.kind]}</span>

      <code className="mt-2 block truncate rounded-md bg-surface-soft px-2 py-1 font-mono text-[11px] text-muted-foreground" title={name}>
        {name}
      </code>
...
function AssetModal({ asset, cat, done, onToggle, onClose }: { asset: Asset; cat?: Category; done: boolean; onToggle: (v: boolean) => void; onClose: () => void }) {
  const name = fileName(asset);
  const prompt = useMemo(() => buildPrompt(asset), [asset]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col rounded-2xl border border-border bg-surface p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-2">
              ...
              {cat && (
                <span className="flex shrink-0 items-center gap-1 rounded-full border border-border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                  <span className="h-2 w-2 rounded-full" style={{ background: DOT[cat.block] }} /> {cat.label} · {cat.zone}
                </span>
              )}
              <span className="shrink-0 rounded-full bg-accent/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-accent">{KIND_LABEL[asset.kind]}</span>
            </div>
        ...
        <label className="mt-3 flex w-fit cursor-pointer items-center gap-2 rounded-lg border border-border px-3 py-1.5 text-sm font-medium">
          <input type="checkbox" checked={done} onChange={(e) => onToggle(e.target.checked)} className="h-4 w-4 accent-[color:var(--color-accent)]" />
        ...
        <div className="mt-4 min-h-0 flex-1 space-y-4 overflow-auto pr-1">
          ...
          <div className="rounded-xl bg-surface-soft p-4">
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Prompt — 1er asset / ancre</span>
              <CopyBtn text={prompt} label="Copier le prompt" primary />
            </div>
            <pre className="whitespace-pre-wrap break-words text-sm leading-relaxed text-foreground/90">{prompt}</pre>
          </div>

          {/* Variante cohérence (avec image de référence) */}
          <div className="rounded-xl border border-dashed border-border bg-surface p-4">
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Variante cohérence — joins l’ancre</span>
              <CopyBtn text={refPrompt} label="Copier la variante" />
            </div>
            <pre className="whitespace-pre-wrap break-words text-sm leading-relaxed text-foreground/80">{refPrompt}</pre>
          </div>

          <p className="border-t border-border-soft pt-3 text-[11px] leading-snug text-muted-foreground">{METHOD_NOTE}</p>
        </div>
      </div>
    </div>
  );
    ...
    return (
      <button
        ...
        className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition ${
          active ? 'bg-surface-soft font-semibold' : 'text-muted-foreground hover:bg-surface-soft hover:text-foreground'
        }`}
      >
        {dot ? (
        ...
        <span className={`shrink-0 text-xs tabular-nums ${d === t && t > 0 ? 'text-success' : 'text-muted-foreground'}`}>
          {d}/{t}
        </span>
      </button>
    );
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-border bg-surface">
        ...
        <nav className="mt-2 flex-1 space-y-0.5 overflow-auto px-3 py-1">
          ...
          <div className="mb-1 mt-3 flex items-center gap-2 px-3">
            ...
            <span className="ml-auto text-[10px] tabular-nums text-muted-foreground">{doneCount}/{total}</span>
          </div>
          {CATEGORIES.map((c) => navItem(c.id, c.label, catDone(c.id), assetsByCategory(c.id).length, DOT[c.block]))}
        </nav>

        <div className="border-t border-border px-4 py-3">
          <label className="flex cursor-pointer items-center gap-2 text-xs font-medium text-muted-foreground">
            <input type="checkbox" checked={hideDone} onChange={(e) => setHideDone(e.target.checked)} className="h-3.5 w-3.5 accent-[color:var(--color-accent)]" />
            Masquer les créés
          </label>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="px-6 py-5">
              ...
              <section key={c.id} className="mb-7">
                {cat === 'all' && (
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="rounded-full border border-border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{c.zone}</span>
                    <span className="ml-auto text-sm font-semibold tabular-nums text-muted-foreground">{catDone(c.id)}/{assetsByCategory(c.id).length}</span>
                  </div>
                )}
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {list.map((a) => (
                    <AssetCard key={a.id} asset={a} done={!!done[fileName(a)]} onToggle={(v) => toggle(a, v)} onOpen={() => setModal(a)} />
                  ))}
                </div>
              </section>
            );
          ...
          {!loaded && <p className="py-12 text-center text-sm text-muted-foreground">Chargement…</p>}
        </div>
      </main>
