...
import {
  Application, Container, Mesh, Geometry, Texture, Assets, Sprite, Text, Graphics, Rectangle, Ticker,
} from 'pixi.js';

// Géométrie iso (mêmes constantes que la salle DOM).
const TW = 72, TH = 36, WALL = 96;
...
export interface PixiCompanion { id: string; name: string; skinUrl: string; size: number }

interface Bee {
  id: string; name: string;
  sprite: Sprite; label: Text;
  bubble: Container; bubbleBg: Graphics; bubbleText: Text; bubbleStr: string;
  c: number; r: number; tc: number | null; tr: number | null;
  facing: 'left' | 'right'; nextAt: number;
  anim: string; frame: number; frameAt: number;
...
function sameOriginAsset(url: string): string {
  ...
  try {
    ...
    if (u.hostname.endsWith('.dowze.ch') && /\/(pets|textures|furniture)\//.test(u.pathname)) return u.pathname;
    return url;
  } catch { return url; }
}
...
function tintForHour(h: number): number {
  ...
  const r = lerp(255, 90), g = lerp(255, 100), b = lerp(255, 160);
  return (r << 16) | (g << 8) | b;
}

export function PixiOpenspace({ size, floorTex, wallTex, companions, speeches, hour, selectedId, roomKey, furniture, editing, gather, onPlaceTile, onCompanionClick }: {
  size: number;
  floorTex: string;
  wallTex?: string;
  ...
  const winHandlersRef = useRef<{ onPointerUp: (e: PointerEvent) => void; onPointerMove: (e: PointerEvent) => void } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      await app.init({ backgroundAlpha: 0, antialias: true, resizeTo: hostRef.current ?? undefined });
      ...
      const onPointerUp = (e: PointerEvent) => {
        ...
        dragging = false; el.style.cursor = editingRef.current ? 'crosshair' : 'grab';
      };
      const onPointerMove = (e: PointerEvent) => { if (!dragging) return; moved += Math.abs(e.clientX - lastX) + Math.abs(e.clientY - lastY); world.x += e.clientX - lastX; world.y += e.clientY - lastY; lastX = e.clientX; lastY = e.clientY; };
      el.addEventListener('pointerdown', (e) => { dragging = true; lastX = e.clientX; lastY = e.clientY; moved = 0; el.style.cursor = 'grabbing'; });
      window.addEventListener('pointerup', onPointerUp);
      window.addEventListener('pointermove', onPointerMove);
      winHandlersRef.current = { onPointerUp, onPointerMove };
      el.addEventListener('wheel', (e) => { e.preventDefault(); const f = e.deltaY < 0 ? 1.1 : 0.9; world.scale.set(Math.max(0.03, Math.min(2.5, world.scale.x * f))); }, { passive: false });
      ...
      ring.ellipse(0, 0, 30, 15).fill({ color: 0x38bdf8, alpha: 0.22 }).stroke({ color: 0x0ea5e9, width: 3.5, alpha: 1 });
      ring.visible = false;
      world.addChild(ring);
      ...
      const tick = (t: Ticker) => {
        ...
        if (ringG) {
          ...
          if (sel) {
            const rx = (sel.c - sel.r) * (TW / 2), ry = (sel.c + sel.r) * (TH / 2);
            const ws = world.scale.x || 1; // contre-échelle → l'anneau garde ~30px à l'écran quel que soit le zoom
            ringG.visible = true; ringG.x = rx; ringG.y = ry; ringG.zIndex = Math.round((sel.c + sel.r) * 100) - 1;
        ...
        for (const b of beesRef.current.values()) {
          ...
          if (b.tc == null) {
            if (!b.hold && now >= b.nextAt) { const N = sizeRef.current; b.tc = Math.floor(Math.random() * N); b.tr = Math.floor(Math.random() * N); b.facing = (b.tc - b.tr) - (b.c - b.r) >= 0 ? 'right' : 'left'; }
          } else {
            const dc = (b.tc ?? b.c) - b.c, dr = (b.tr ?? b.r) - b.r; const d = Math.hypot(dc, dr) || 1;
            if (d < 0.06) { b.c = b.tc!; b.r = b.tr!; b.tc = null; b.tr = null; b.nextAt = b.hold ? Infinity : now + 2500 + Math.random() * 6000; }
            else { const step = Math.min(d, 2.6 * dt); b.c += (dc / d) * step; b.r += (dr / d) * step; }
          }
          const moving = b.tc != null;
          const anim = moving ? (b.facing === 'right' ? 'running-right' : 'running-left') : 'idle';
          const spec = ANIMS[anim] ?? ANIMS.idle!;
          if (b.anim !== anim) { b.anim = anim; b.frame = 0; b.frameAt = now; }
          if (now - b.frameAt >= spec.ms / spec.frames) { b.frame = (b.frame + 1) % spec.frames; b.frameAt = now; }
          // position iso + tri de profondeur
          const x = (b.c - b.r) * (TW / 2), y = (b.c + b.r) * (TH / 2);
          b.sprite.x = x; b.sprite.y = y; b.sprite.zIndex = Math.round((b.c + b.r) * 100);
          b.label.x = x; b.label.y = y - b.sprite.height - 6; b.label.zIndex = b.sprite.zIndex + 1;
          // frame courante
          if (b.baseTex) {
          ...
          if (str !== b.bubbleStr) {
            ...
            if (str) {
              ...
              const w = Math.min(240, b.bubbleText.width) + 16, h = b.bubbleText.height + 12;
              b.bubbleText.x = -w / 2 + 8; b.bubbleText.y = -h + 6;
              b.bubbleBg.clear();
              b.bubbleBg.roundRect(-w / 2, -h, w, h, 8).fill(0xffffff).stroke({ color: 0x000000, alpha: 0.12, width: 1 });
            }
          }
          if (b.bubble.visible) { b.bubble.x = x; b.bubble.y = y - b.sprite.height - 20; b.bubble.zIndex = b.sprite.zIndex + 2; }
        }
      };
      app.ticker.add(tick);
    ...
    return () => {
      destroyed = true; setReady(false);
      const h = winHandlersRef.current;
      if (h) { window.removeEventListener('pointerup', h.onPointerUp); window.removeEventListener('pointermove', h.onPointerMove); winHandlersRef.current = null; }
      const a = appRef.current; appRef.current = null;
      if (a) { try { a.destroy(true, { children: true }); } catch { /* */ } }
      beesRef.current.clear(); furnRef.current.clear(); decorRef.current = [];
    };
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const ch of [...world.children]) { if ((ch as Container).label === 'decor') { try { (ch as Container).destroy({ children: true }); } catch { /* */ } } }
      decorRef.current = [];
      const decor = new Container(); decor.label = 'decor'; decor.zIndex = -1_000_000; world.addChildAt(decor, 0);

      // ---- SOL : Mesh iso tuilé (une géométrie GPU) ----
      const N = size;
      const positions: number[] = [], uvs: number[] = [], indices: number[] = [];
      for (let r = 0; r <= N; r++) for (let c = 0; c <= N; c++) { positions.push((c - r) * (TW / 2), (c + r) * (TH / 2)); uvs.push(c, r); }
      const stride = N + 1;
      for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) { const a = r * stride + c; indices.push(a, a + 1, a + stride, a + 1, a + stride + 1, a + stride); }
      const geo = new Geometry({ attributes: { aPosition: { buffer: new Float32Array(positions), format: 'float32x2' }, aUV: { buffer: new Float32Array(uvs), format: 'float32x2' } }, indexBuffer: new Uint32Array(indices) });
      ...
      if (wallTex) {
        ...
        const wall = (left: boolean) => {
          ...
          const P = left
            ? [[0, -WALL], [-N * (TW / 2), N * (TH / 2) - WALL], [-N * (TW / 2), N * (TH / 2)], [0, 0]]
            : [[0, -WALL], [N * (TW / 2), N * (TH / 2) - WALL], [N * (TW / 2), N * (TH / 2)], [0, 0]];
          const pos: number[] = []; for (const [x, y] of P) pos.push(x!, y!);
          // UV : tuilé le long du mur (N segments) × hauteur (WALL/TH tuiles).
          const uvH = WALL / TH;
          const uv = left ? [0, 0, N, 0, N, uvH, 0, uvH] : [0, 0, N, 0, N, uvH, 0, uvH];
          const g = new Geometry({ attributes: { aPosition: { buffer: new Float32Array(pos), format: 'float32x2' }, aUV: { buffer: new Float32Array(uv), format: 'float32x2' } }, indexBuffer: new Uint32Array([0, 1, 2, 0, 2, 3]) });
          const m = new Mesh({ geometry: g, texture: wtex });
          m.zIndex = -2_000_000 + (left ? 0 : 1);
          void sx;
          return m;
        };
        const wl = wall(true), wr = wall(false);
        decor.addChild(wl); decor.addChild(wr);
      ...
      const sc = Math.max(0.03, Math.min(1.2, Math.min(vw / (N * TW + 1), vh / (N * TH + WALL + 1)) * 0.85));
      world.scale.set(sc);
      world.x = vw / 2;
  ...
  useEffect(() => {
    ...
    const N = sizeRef.current;
    for (const b of beesRef.current.values()) {
      b.c = Math.min(b.c, N - 1); b.r = Math.min(b.r, N - 1);
      if (b.tc != null) b.tc = Math.min(b.tc, N - 1);
      if (b.tr != null) b.tr = Math.min(b.tr, N - 1);
    }
    const sel = selectedRef.current ? beesRef.current.get(selectedRef.current) : null;
    if (sel) { sel.c = N - 1; sel.r = N - 1; sel.tc = Math.floor(N / 2); sel.tr = Math.floor(N / 2); sel.facing = 'left'; sel.nextAt = 0; }
  }, [ready, roomKey]);
  ...
  useEffect(() => {
    ...
    if (!gather) {
      ...
      for (const b of bees.values()) { if (b.hold) { b.hold = false; b.tc = null; b.tr = null; b.nextAt = now + Math.random() * 1200; } }
      return;
    }
    const leader = bees.get(gather.leaderId);
    if (!leader) return;
    const N = sizeRef.current;
    leader.hold = true; leader.tc = null; leader.tr = null; leader.nextAt = Infinity; // le leader reste en place
    ...
    ids.forEach((id, i) => {
      ...
      b.hold = true; b.tc = tc; b.tr = tr; b.facing = (tc - tr) - (b.c - b.r) >= 0 ? 'right' : 'left';
    });
  }, [ready, gather]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // retire les partis
      for (const [id, b] of [...bees]) { if (!wanted.has(id)) { b.sprite.destroy(); b.label.destroy(); b.bubble.destroy({ children: true }); bees.delete(id); } }
      // ajoute les nouveaux
      for (const comp of companions) {
        ...
        if (cancelled) return;
        const spr = new Sprite(base ? new Texture({ source: (base as Texture).source, frame: new Rectangle(0, 0, FRAME_W, FRAME_H) }) : Texture.WHITE);
        ...
        const label = new Text({ text: comp.name, style: { fontFamily: 'Inter, sans-serif', fontSize: 22, fill: '#ffffff', stroke: { color: 'rgba(0,0,0,0.55)', width: 6 } } });
        label.anchor.set(0.5, 1);
        // bulle de dialogue (cachée par défaut)
        const bubble = new Container(); bubble.visible = false;
        const bubbleBg = new Graphics();
        const bubbleText = new Text({ text: '', style: { fontFamily: 'Inter, sans-serif', fontSize: 18, fill: '#334155', wordWrap: true, wordWrapWidth: 224 } });
        bubble.addChild(bubbleBg); bubble.addChild(bubbleText);
        world.addChild(spr); world.addChild(label); world.addChild(bubble);
        bees.set(comp.id, { id: comp.id, name: comp.name, sprite: spr, label, bubble, bubbleBg, bubbleText, bubbleStr: '', c: Math.random() * size, r: Math.random() * size, tc: null, tr: null, facing: 'right', nextAt: 0, anim: 'idle', frame: 0, frameAt: 0, baseTex: base as Texture | null });
      }
    })();
    return () => { cancelled = true; };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const [key, spr] of [...map]) { if (!wanted.has(key)) { spr.destroy(); map.delete(key); } }
      for (const f of list) {
        ...
        const x = (f.c - f.r) * (TW / 2), y = (f.c + f.r) * (TH / 2);
        spr.x = x; spr.y = y; spr.zIndex = Math.round((f.c + f.r) * 100);
        world.addChild(spr);
        map.set(key, spr);
      }
    })();
