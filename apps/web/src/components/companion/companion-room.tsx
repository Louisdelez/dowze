...
export function CompanionRoom() {
  ...
  const dialoguePages = useCallback((raw: string): string[] => {
    const natural = raw
      ...
      .replace(/\b(\d+)\s*c\.\s*à\s*s\.?(?=\s|$|[,;:])/gi, '$1 cuillère à soupe')
      .replace(/\b(\d+)\s*c\.\s*à\s*c\.?(?=\s|$|[,;:])/gi, '$1 cuillère à café')
      .replace(/\b1\s*c\.(?=\s|$|[,;:])/gi, 'une cuillère')
      .replace(/\b(\d+)\s*c\.(?=\s|$|[,;:])/gi, '$1 cuillères')
