...
export default function Reglages() {
  ...
  useEffect(() => { if (ready && signedIn) void refresh(); }, [ready, signedIn, refresh]);
  ...
  if (!signedIn || !plugin?.activation?.enabled) {
    return (
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <a href="/" className="mt-2 inline-flex text-sm font-medium text-accent hover:underline">← Retour</a>
      </div>
    );
  }
  ...
  async function save(next: Partial<AlimConfig>) {
    ...
    setMsg(null);
    try {
      const merged: AlimConfig = { mealsPerDay: next.mealsPerDay ?? mealsPerDay, prepPerWeek: next.prepPerWeek ?? prepPerWeek };
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <div className="mt-3 flex gap-2">
          {[2, 3, 4].map((n) => (
            <button key={n} disabled={busy} onClick={() => save({ mealsPerDay: n })} className={`h-11 w-11 rounded-lg border text-sm font-medium transition ${mealsPerDay === n ? 'border-accent bg-accent text-accent-foreground' : 'border-border hover:bg-muted'}`}>{n}</button>
          ))}
        </div>
        <h2 className="mt-4 text-sm font-semibold">Meal-prep par semaine</h2>
        <div className="mt-3 flex gap-2">
          {[1, 2, 3].map((n) => (
            <button key={n} disabled={busy} onClick={() => save({ prepPerWeek: n })} className={`h-11 w-11 rounded-lg border text-sm font-medium transition ${prepPerWeek === n ? 'border-accent bg-accent text-accent-foreground' : 'border-border hover:bg-muted'}`}>{n}×</button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Retire le meal-prep du planning et révoque les accords.</p>
        <button onClick={deactivate} disabled={busy} className="mt-3 rounded-lg border border-red-200 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 disabled:opacity-50">Désactiver</button>
      </div>
    </div>
  );
}
