...
export default function Reglages() {
  ...
  async function save(next: Partial<SportsConfig>) {
    ...
    setMsg(null);
    try {
      const merged: SportsConfig = { discipline: next.discipline ?? discipline, frequencyPerWeek: next.frequencyPerWeek ?? goal };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <select value={discipline} disabled={busy} onChange={(e) => save({ discipline: e.target.value })} className="mt-3 w-full rounded-lg border border-border px-3 py-2 text-sm capitalize">
        ...
        <div className="mt-3 flex gap-2">
          {[2, 3, 4, 5].map((n) => (
            <button key={n} disabled={busy} onClick={() => save({ frequencyPerWeek: n })} className={`h-11 w-11 rounded-lg border text-sm font-medium transition ${goal === n ? 'border-accent bg-accent text-accent-foreground' : 'border-border hover:bg-muted'}`}>{n}×</button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Retire tes entraînements du planning et révoque les accords.</p>
